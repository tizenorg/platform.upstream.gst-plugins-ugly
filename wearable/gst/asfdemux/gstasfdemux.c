    ...
    GstClockTime seek_time, GstClockTime * p_idx_time, guint * speed)
{
  ...
  *packet = demux->sidx_entries[idx].packet;
  ...
  } while((demux->packet_send_time - seek_time) >= GST_SECOND && demux->is_seeked && (demux->packet_send_time > seek_time));
...
gst_asf_demux_chain_headers (GstASFDemux * demux)
{
  ...
  if (!data) {
    GST_ERROR_OBJECT(demux, "Failed to get inputbuffer from adapter\n");
...
#ifdef ASFDEMUX_MODIFICATION
    if(ret != GST_FLOW_OK || ret == GST_FLOW_WRONG_STATE) {
      GST_INFO("unreffing the non pushed buffer %p", payload->buf);
    ...
    guint64 * p_size)
{
  ...
  switch (obj.id) {
    ...
    case ASF_OBJ_SIMPLE_INDEX:
      ...
      break;
#ifdef ASFDEMUX_MODIFICATION
    case ASF_OBJ_PLAYREADY_ENCRYPTION_OBJECT:
#endif
